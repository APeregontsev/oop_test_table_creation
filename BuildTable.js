class BuildTable {
  /**
   *  _tableClass - css классы оформления
   *  data  - выходные данные
   *  attribute - что выводим, названия столбцов
   *  _element  - куда выводить таблицу
   *  _header  - заголовок таблицы
   *  _headerClass  - css классы заголовка
   */
  ...
  render(data) {
    ...
    // lets create & show table
    const table = document.createElement("table");
    this._tableClass.forEach((cssClass) => {
      table.classList.add(cssClass);
    });

    // lets create table header
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      for (let key in this.attribute) {
        ...
        // атрибут src -- если присутствует -> выводим как html то есть с разметкой <br>
